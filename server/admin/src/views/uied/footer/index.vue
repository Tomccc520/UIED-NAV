...
<template>
    <div class="footer-setting">
            ...
            <el-tabs v-model="activeTab">
                ...
                <el-tab-pane label="页脚链接" name="links">
                    <div class="mb-4 flex gap-4">
                        ...
                        <el-select v-model="linkGroupFilter" placeholder="筛选分组" clearable @change="getLinkLists">
                    ...
                    <el-table size="large" v-loading="linkPager.loading" :data="linkPager.lists">
                        ...
                        <el-table-column label="操作" width="120" fixed="right">
                            <template #default="{ row }">
                                <el-button type="primary" link @click="handleEditLink(row)">编辑</el-button>
                                <el-button type="danger" link @click="handleDeleteLink(row.id)">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
        ...
        <!-- 链接编辑弹窗 -->
        <el-dialog v-model="showLinkEdit" :title="linkData.id ? '编辑链接' : '添加链接'" width="500px">
            <el-form ref="linkFormRef" :model="linkData" :rules="linkRules" label-width="80px">
                <el-form-item label="所属分组" prop="groupId">
                    <el-select v-model="linkData.groupId" placeholder="请选择分组" style="width: 100%">
                        <el-option v-for="g in groupOptions" :key="g.id" :label="g.name" :value="g.id" />
                    </el-select>
                </el-form-item>
            ...
            <template #footer>
                ...
                <el-button type="primary" :loading="linkLoading" @click="handleSubmitLink">确定</el-button>
            </template>
        </el-dialog>
    </div>
</template>
...
import { uiedFooterGroupList, uiedFooterGroupAdd, uiedFooterGroupEdit, uiedFooterGroupDelete,
    uiedFooterLinkList, uiedFooterLinkAdd, uiedFooterLinkEdit, uiedFooterLinkDelete } from '@/api/uied'
...
// 链接
const { pager: linkPager, getLists: getLinkListsBase } = usePaging({ fetchFun: uiedFooterLinkList })
const getLinkLists = () => getLinkListsBase({ groupId: linkGroupFilter.value })
const showLinkEdit = ref(false)
const linkLoading = ref(false)
const linkFormRef = ref<FormInstance>()
const linkData = reactive({ id: 0, groupId: undefined as number | undefined, name: '', url: '', icon: '', sortOrder: 0, openInNewTab: false, isActive: true })
...
// 分组操作
const handleAddGroup = () => { Object.assign(groupData, { id: 0, name: '', sortOrder: 0, isActive: true }); showGroupEdit.value = true }
const handleEditGroup = (row: any) => { Object.assign(groupData, row); showGroupEdit.value = true }
const handleSubmitGroup = async () => {
    await groupFormRef.value?.validate()
    groupLoading.value = true
    try {
        if (groupData.id) { await uiedFooterGroupEdit(groupData); feedback.msgSuccess('编辑成功') }
        else { await uiedFooterGroupAdd(groupData); feedback.msgSuccess('添加成功') }
...
// 链接操作
const handleAddLink = () => { Object.assign(linkData, { id: 0, groupId: undefined, name: '', url: '', icon: '', sortOrder: 0, openInNewTab: false, isActive: true }); showLinkEdit.value = true }
const handleEditLink = (row: any) => { Object.assign(linkData, row); showLinkEdit.value = true }
const handleSubmitLink = async () => {
    await linkFormRef.value?.validate()
    linkLoading.value = true
    try {
        if (linkData.id) { await uiedFooterLinkEdit(linkData); feedback.msgSuccess('编辑成功') }
        else { await uiedFooterLinkAdd(linkData); feedback.msgSuccess('添加成功') }
