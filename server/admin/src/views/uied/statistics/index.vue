<template>
  <div class="statistics-container">
    ...
    <el-row :gutter="16" class="mb-4">
      <el-col :span="6">
        <el-card shadow="never">
          <el-statistic title="热门网站" :value="overview.website?.hotCount || 0" value-style="color: #f56c6c" />
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never">
          <el-statistic title="推荐网站" :value="overview.website?.featuredCount || 0" value-style="color: #e6a23c" />
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never">
          <el-statistic title="新网站" :value="overview.website?.newCount || 0" value-style="color: #67c23a" />
        </el-card>
      </el-col>
      <el-col :span="6">
        <el-card shadow="never">
          <el-statistic title="点击率" :value="overview.website?.clickRate || 0" suffix="%" />
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-tabs v-model="activeTab">
        ...
        <el-row :gutter="16">
          <el-col :span="14">
            <el-card shadow="never">
              ...
              <el-table :data="clickStats.topWebsites" v-loading="loading" max-height="500">
                ...
                <el-table-column prop="clickCount" label="点击量" width="100" sortable>
                  <template #default="{ row }">
                    <span style="color: #409eff; font-weight: bold;">{{ row.clickCount?.toLocaleString() }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="操作" width="80">
                  <template #default="{ row }">
                    <el-link :href="row.url" target="_blank" type="primary">访问</el-link>
                  </template>
                </el-table-column>
              </el-table>
            </el-card>
          ...
          <el-col :span="10">
            <el-card shadow="never">
              ...
              <el-table :data="clickStats.categoryStats?.slice(0, 15)" v-loading="loading" max-height="500">
                <el-table-column prop="name" label="分类" min-width="120" show-overflow-tooltip />
                <el-table-column prop="websiteCount" label="网站数" width="80" sortable />
                <el-table-column prop="clickCount" label="总点击" width="100" sortable>
                  <template #default="{ row }">
                    <span style="color: #67c23a;">{{ row.clickCount?.toLocaleString() }}</span>
                  </template>
                </el-table-column>
              </el-table>
            </el-card>
      ...
      <el-tab-pane label="搜索统计" name="search">
        <el-row :gutter="16" class="mb-4">
          <el-col :span="6">
            <el-card shadow="never">
              <el-statistic title="总搜索次数" :value="searchStats.totalSearches || 0" />
            </el-card>
          </el-col>
          <el-col :span="6">
            <el-card shadow="never">
          ...
          <el-col :span="6">
            <el-card shadow="never">
              <el-statistic title="AI搜索占比" :value="searchStats.aiRatio || 0" suffix="%" />
            </el-card>
          </el-col>
          <el-col :span="6">
            <el-card shadow="never">
              <el-statistic title="热门搜索词" :value="searchStats.topSearches?.length || 0" />
            </el-card>
          </el-col>
        </el-row>
        ...
        <el-card shadow="never">
          ...
          <el-table :data="searchStats.topSearches" v-loading="searchLoading">
            ...
            <el-table-column prop="count" label="搜索次数" width="120">
              <template #default="{ row }">
                <span style="color: #409eff; font-weight: bold;">{{ Number(row.count).toLocaleString() }}</span>
              </template>
            </el-table-column>
          </el-table>
...
import { Link, View, Folder, Document } from '@element-plus/icons-vue'
